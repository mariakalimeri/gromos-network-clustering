gromos_clus_for_prob_dist <- function(distanceMatrix, densitiesObj, cutoff) {
  centroid <- list()
  centroidVector <- NULL
  ...
  while (max(rowSummation) != 0) {
    clusteredConformationsSoFar <-
      clusteredConformationsSoFar + max(rowSummation)
    print(paste(
      100 * (clusteredConformationsSoFar / numberOfFrames),
      "% of conformations clustered"
    ))
  ...
  return(list(
    numberOfClusters = max(membershipVector),
    membershipVector = membershipVector,
    centroidDistribution = centroid,
    centroidPositions = centroidVector
  ))
}
