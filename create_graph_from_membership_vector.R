...
mem <- as.matrix(read.table(
  "../T260-ala-42/clusteringData/membershipVector_1.00.dat"
))
...
# ATTENTION!
# Some older igraph distributions count the vertex indices from 0 and not from
# 1. The lines below are for one-based indices.
# To check that you are fine with your distribution, after creating the myG 
# graph object, do min(V(myG))
...
myG <- 
  set.edge.attribute(myG, "Weight", value = simplifiedWeightedEdgeList[, 3])
...
# write.table(
#   simplifiedWeightedEdgeList,
#   file=paste(outname,".cg.mcledgelist.txt", sep=""),
#   col.names=F,
#   row.names=F
# )
...
write.graph(
  myG,
  file = paste(outname, ".graphml", sep = ""),
  format = c("graphml")
)
...
#
# simplifiedWeightedEdgeList <-
#   simplifiedWeightedEdgeList[order(simplifiedWeightedEdgeList[,1]),]
# 
# for (i in 1:max(simplifiedWeightedEdgeList)){
#    normConst <- 
#      sum(simplifiedWeightedEdgeList[which(simplifiedWeightedEdgeList[,1]==i, arr.ind=T),3])
#    simplifiedWeightedEdgeList[which(simplifiedWeightedEdgeList[,1]==i, arr.ind=T),3] <-
#      simplifiedWeightedEdgeList[which(simplifiedWeightedEdgeList[,1]==i, arr.ind=T),3]/normConst
# }
# 
