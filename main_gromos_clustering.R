...
#
# WHAT IT DOES:
# Cluster, i.e. group together according to similarity, snapshots of a dynamic
# network. I wrote and used this script for the output trajectory of a Molecular
# Dynamics simulation. However, as long as the input format is suitable, the
# script can be used for any kind of network.
...
# trajectory frames, i.e. the hydration shell has variable number of waters
# since they are moving, the maximum number of pairs is arbitrarily set to
# some "large" number N (e.g. 3000).
...
# 3*N and so on. The line number N must be given below as an input. See
# USER INPUT.
...
# Use only one value of clustering on a second run in order to create the
# networks. Don't use only one value of the clusteringCutoff if you are not
# sure whether it create few clusters. One value of clusteringCutoff triggers
# the creation of centroid graphs and it will greate a huge number of files if
# the cutoff is fine.
clusteringCutoff <- c(0.15, 0.20, 0.25, 0.30, 0.35, 0.40)
# ============= END INPUT ======================================================

# Loading all the needed scripts. They all must be in the same folder
source("data_to_graph.R")
source("jensen_shannon.R")
source("bandwidth_calculation_for_density.R")
source("calculate_distance_matrix.R")
source("gromos_clus_for_prob_dist.R")
source("create_centroid_graphs.R")
...
for (jh in 1:length(cases)) {
  ...
  # -> data_to_graph.R
  # -> jensen_shannon.R
  # -> bandwidth_calculation_for_density.R
  # -> calculate_distance_matrix.R
  ...
  if (file.exists("distanceMatrix_and_densities.RData")) {
    ...
    graph.from.mat <- tryCatch(
      data_to_graph(
        mat,
        numberOfFrames = numberOfFrames,
        arbitrarilySetNoOfPairs = arbitrarilySetNoOfPairs
      ),
    ...
    write.table(
      graph.from.mat$transitivityVec,
      paste("networkAnalysisData/transitivityVec_", outname, ".dat", sep = ""),
      col.names = F,
      row.names = F
    )
    ...
    write.table(
      graph.from.mat$diameterVec,
      paste("networkAnalysisData/diameterVec_", outname, ".dat", sep = ""),
      col.names = F,
      row.names = F
    )
    ...
    write.table(
      graph.from.mat$average.path.length.vec.all,
      paste(
        "networkAnalysisData/average.path.length.vec.all_",
        outname,
        ".dat",
        sep = ""
      ),
    ...
    write.table(
      graph.from.mat$average.path.length.vec.onlyConn, 
      paste(
        "networkAnalysisData/average.path.length.vec.onlyConn_", 
        outname, 
        ".dat", 
        sep = ""
      ), 
    ...
    write.table(
      graph.from.mat$average.degree, 
      paste("networkAnalysisData/average.degree_", outname, ".dat", sep = ""), 
      col.names = F, 
      row.names = F
    )

    # Use explicitly a different algorithm for the quantile estimation for the 
    # bw of the density as the default fails for a large set of our data
    bndwd <- lapply(
      graph.from.mat$laplacian.spectrum, 
      bandwidth_calculation_for_density
    )
    densitiesObj <- NULL
    for (i in 1:length(graph.from.mat$laplacian.spectrum)) {
      print(paste("Spectrum:", i))
      densitiesObj[[i]] <- tryCatch(
        density(
          graph.from.mat$laplacian.spectrum[[i]], 
          bw = bndwd[[i]])[[2]], 
        error = function(e) NULL
      )
    }
    ...
    distanceMatrix <- tryCatch(
      calculate_distance_matrix(densitiesObj), 
      error = function(e) NULL
    )
    save(
      distanceMatrix, 
      densitiesObj, 
      numberOfFrames, 
      file = "distanceMatrix_and_densities.RData"
    )
  }
  ...
  if (length(dummy) != 0) {
    # NOTE (Tuesday 29/03/2016): When the networks are small (some 10 nodes or
    # less) the density estimates may be wrongly estimated if there exist a few 
    # too many eigenvalues equal to 1. The reason is that for the density 
    # estimation the quantile calculation is off for too many ones (quantiles 
    # collapse to the same value). Therefore, some densities might be completely 
    # off and their respective distances from all other frames will appear as NaN
  ...
  for (i in 1:length(clusteringCutoff)) {
    ...
    print(paste(
      "Doing the gromos clustering for cutoff", 
      clusteringCutoff[i], "...")
    )
    clusGrom <- tryCatch(
      gromos_clus_for_prob_dist(
        distanceMatrix, 
        densitiesObj, 
        cutoff = clusteringCutoff[i]), 
      error = function(e) NULL
    )
    print("Writing output files...")
    outNameComp <- format(round(clusteringCutoff[i], 2), nsmall = 2)
    write.table(
      clusGrom$membershipVector, 
      paste("clusteringData/membershipVector_", outNameComp, ".dat", sep = ""), 
      col.names = F, 
      row.names = F
    )
    write.table(
      clusGrom$centroidPositions, 
      paste("clusteringData/centroidPositions_", outNameComp, ".dat", sep = ""), 
      col.names = F, 
      row.names = F
    )
    # ====== END CLUSTERING ====================================================
    ...
    ## ATTENTION: Use this only when the number of clusters is small, up to 10. 
    ## It creates and writes down a network in a graphml file
    ...
    if (length(clusteringCutoff) == 1) {
      ...
      outname <- substring(cases[jh], 4)
      centroid.graphs <- create_centroid_graphs(
        mat = mat, 
        frameNumbersOfCentroids = clusGrom$centroidPositions, 
        numberOfFrames = numberOfFrames, 
        arbitrarilySetNoOfPairs = arbitrarilySetNoOfPairs, 
        outname, 
        clusteringCutoff
    ...
    if (!is.null(clusGrom)) {
      pdf(
        file = paste(
          "clusteringData/clusterMemberships_", 
          outNameComp, 
          ".pdf", 
          sep = ""
        ), 
